...
def get_cache(supabase, key):
    if not supabase:
        return None
    try:
        resp = supabase.table("cache_music_info") \
                      .select("data") \
                      .eq("key", key) \
                      .maybe_single() \
                      .execute()
...
def set_cache(supabase, key, data):
    if supabase:
        supabase.table("cache_music_info") \
                .upsert({"key": key, "data": data}, on_conflict="key") \
                .execute()
...
async def scrape_license(http, query):
    tasks = [http.get(u.format(q=query)) for u in LYRIC_SITES]
...
async def get_youtube_info(youtube, vid):
    ...
    s, st = data.get("snippet", {}), data.get("status", {})
    return {
        "Title": s.get("title"),
...
async def run(args):
    ...
    key = make_cache_key(args.command, getattr(args,'query',''), getattr(args,'file',''))
    res = get_cache(supabase, key) if supabase else None
    if res:
        display(res)
        await http.aclose()
        return
    ...
    if args.command == "youtube":
        ...
        res["google"] = await google_search_license(
            http,
    ...
    if "youtube" in res:
        desc = res["youtube"].get("Description","")
        res["nlp"] = nlp_classify(desc)
    if "youtube" in res or "file_metadata" in res:
        res["copyright_status"] = determine_status(
            res.get("youtube",{}),
            res.get("file_metadata",{})
        )

    display(res)
    set_cache(supabase, key, res)
    await http.aclose()
